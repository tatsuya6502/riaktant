...
var syslogHost = "localhost";
var syslogPort = 10514;
var numMessagesToGenerate = 1000;
...
for (var i = 0; i < numMessagesToGenerate; i++) {
  ...
  ++activeSessions;
  client.send(message, 0, message.length, syslogPort, syslogHost, function(err, bytes) {
    if (err == null) {
      ++messagesSent;
    }
    --activeSessions;
    if (activeSessions <= 0) {
      client.close();
      console.log("Sent %d messages.", messagesSent);
    }  
  });
